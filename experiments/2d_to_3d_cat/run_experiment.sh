...
CORRECTION_MANIFEST="${ANIMAL_DATA_ROOT}/outputs/manual_correction/cat_walk/cvat_export_v2/cvat_manifest.json"
ANNOTATIONS="${ANIMAL_DATA_ROOT}/outputs/manual_correction/cat_walk/cvat_export_v2/annotations_migrated_from_review_v1_clean.xml"
BASELINE_TRAJECTORY="${ANIMAL_DATA_ROOT}/outputs/semantic_six/cat_walk_v2/keypoint_trajectory_2d.json"
IDENTITY_ANCHOR_MANIFEST="${ANIMAL_DATA_ROOT}/outputs/anchor_review/cat_walk_v2/identity_anchor_candidates.json"
...
for source in \
  "$MESH" "$VIDEO" "$PREDICTIONS" "$TRAJECTORY" \
  "$CORRECTION_MANIFEST" "$ANNOTATIONS" "$BASELINE_TRAJECTORY" \
  "$IDENTITY_ANCHOR_MANIFEST" "$PYTHON"; do
  if [[ ! -f "$source" ]]; then
...
PROVENANCE_ARGS=(
  ...
  --script "${REPO_ROOT}/src/qianji_animal_motion/lift_3d.py"
  --script "${REPO_ROOT}/src/qianji_animal_motion/lift_cli.py"
  ...
  --input "$PREDICTIONS"
  --input "$TRAJECTORY"
  --input "$CORRECTION_MANIFEST"
  --input "$ANNOTATIONS"
  --input "$BASELINE_TRAJECTORY"
  --input "$IDENTITY_ANCHOR_MANIFEST"
  --invocation-arg bash
  --invocation-arg "${REPO_ROOT}/experiments/2d_to_3d_cat/run_experiment.sh"
...
for scale in 0.10 0.25 0.50; do
  ...
  PYTHONPATH="${REPO_ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
    ...
    --identity-anchor-manifest "$IDENTITY_ANCHOR_MANIFEST" \
