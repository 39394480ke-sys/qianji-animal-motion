...
- 完整保留 DeepLabCut SuperAnimal-Quadruped 的 39 点二维观测与质量语义。
- 将六点轨迹导入 CVAT 人工复核，再导回独立版本。
- 将 39 点身体相对二维运动重定向为 Mesh 辅助的 2.5D 关键点序列。
- 用明确的观测/控制映射驱动 QianJi 12 节点、30 根杆 VGT 序列。
...
当前主流程已经不是“39 点压缩成六点输出”，而是：

```text
30 FPS 小猫视频 + 从视频帧经 Hunyuan3D 得到的已有 GLB
-> 272 x 39 二维观测和逐点质量
-> 39 个 Mesh/VGT 中性地标
-> 272 x 39 身体相对 2.5D 关键点运动
-> 39 观测角色到 6 个结构控制目标的显式适配
-> QianJi 几何可达性投影
-> 272 x 12 x 3 VGT 节点位置 + 30 根固定连接杆
-> 1246 x 720、30 FPS、272 帧四视图动画
```

这里六点只属于 **VGT 控制接口**，不会替代或丢弃 39 点输出。也不把 12 个
节点传播到 2,154 个 Mesh 顶点。完整原理、文件和真实结果见
[39 点 VGT 流程](docs/39point_vgt_motion_pipeline.md)。
